...
- 【*注*】***这里似乎并不能简单的替换成  right =nums.length - 1， while(left <= right)  的形式***

2. 为什么没有返回 -1 的操作？如果 nums 中不存在 target 这个值，怎么办？

答：因为要一步一步来，先理解一下这个「左侧边界」有什么特殊含义：
![在这里插入图片描述](https://img-blog.csdnimg.cn/3fb4b322c0044f66b7e36e84524eceb8.png?x-oss-process=image/watermark,type_ZHJvaWRzYW5zZmFsbGJhY2s,shadow_50,text_Q1NETiBARGV2aW5fd2hpdGU=,size_20,color_FFFFFF,t_70,g_se,x_16)
...
**也就是说，当数组中有这个数的时候，会返回这个有序数组的最左边等于这个数的下标！而当没有这个target的时候，会返回应该插入这个数的位置，也就是，返回的是第一个比这个target大的元素的下标！！**
...
while循环完后，left=right,nums[left]或者说是nums[right] 是数组中第一个大于target的元素。
所以说，如果数组中有target，那么nums[left-1]就是target。

如果数组中没有target，那么第一个if将永远执行不到，结果就是会和查找左侧边界的时候，数组中没有元素一样，都是第一个if执行不到，所以left和right就是第一个大于target的元素的下标。

所以说nums[left-1]是第一个小于target的元素，nums[left]是第一个大于target的元素。
...
补充一个来源自上面的点，就是当查找的元素不在有序序列中时，返回它将会被按顺序插入的位置。本质上就是查找左侧边界时候的代码！！
**1)法一：**
~~~cpp
